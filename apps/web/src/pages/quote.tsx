    ...
    <div className="container mx-auto px-4 py-12 md:py-20 max-w-5xl">
      <div className="mb-12 md:mb-16">
        ...
        <p className="text-lg text-foreground/70 font-light">
          Select styles for your appointment, or book now and tell us about your space in your message.
        </p>
      </div>
        ...
        <div className="w-full lg:w-2/3 flex flex-col gap-6">
          <div className="bg-secondary/30 border border-border p-6 flex justify-between items-center mb-2">
            <span className="font-medium tracking-wide uppercase text-sm">
              Items Selected for Consultation: {items.length} {items.length === 0 ? '— optional' : ''}
            </span>
          </div>

          {items.length > 0 ? <div className="divide-y divide-border border-b border-border">
            {itemsWithImages.map((item) => (
              <QuoteItemRow key={item.productId} item={item} onRemove={removeItem} />
            ))}
          </div> : <div className="border border-dashed border-border p-8 text-sm text-muted-foreground">No items selected. You can still book a consultation and describe what you need.</div>}
        </div>
